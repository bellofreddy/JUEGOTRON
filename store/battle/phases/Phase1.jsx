// Phase1.jsx — Tron Legacy / Battle Royale — Diseño cinematográfico
// Referencia visual: Tron Legacy (2010) — negro absoluto + líneas de circuito neón
"use client";
...
// Colores de identidad — cada programa tiene su color de circuito
...
export default function Phase1({ onComplete }) {
  ...
  const { bots, playerTakeMotor, botTakeMotor, eliminate } = useBattleStore();
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // ── Escena — negro absoluto como en la película ─────────────
      const scene = new BABYLON.Scene(engine);
      scene.clearColor        = new BABYLON.Color4(0, 0, 0, 1); // negro puro
      scene.ambientColor      = new BABYLON.Color3(0, 0, 0);
      scene.collisionsEnabled = true;
      scene.gravity           = new BABYLON.Vector3(0, -9.81 * 0.025, 0);
      // Sin fog — el negro se funde naturalmente
      ...
      const camera = new BABYLON.UniversalCamera("cam",
        new BABYLON.Vector3(0, 1.8, 8), scene);
      ...
      camera.fov                = 1.1; // FOV más amplio — cinematográfico

      canvasRef.current.addEventListener("click", () => {
        canvasRef.current.requestPointerLock();
      });

      // ── ILUMINACIÓN — solo desde abajo, como el grid ilumina ───
      // Sin luz ambiente — negro absoluto entre líneas
      const gridLight = new BABYLON.PointLight("gridL",
        new BABYLON.Vector3(0, 0.1, -20), scene);
      gridLight.intensity = 6;
      gridLight.diffuse   = new BABYLON.Color3(0, 0.97, 1);
      gridLight.range     = 80;

      // Luz de identidad del jugador — ilumina sus manos desde abajo
      const playerLight = new BABYLON.PointLight("playerL",
        new BABYLON.Vector3(0, 0, 0), scene);
      playerLight.intensity = 3;
      playerLight.diffuse   = new BABYLON.Color3(0, 0.97, 1);
      playerLight.range     = 4;

      // ── GLOW — intenso, como en la película ────────────────────
      const glow = new BABYLON.GlowLayer("glow", scene);
      glow.intensity = 1.8;

      // ── HELPER: crear material de circuito neón ─────────────────
      // Negro con líneas de luz — identidad visual de Tron
      ...
      // ── PISO — negro absoluto con grid de perspectiva ──────────
      ...
      // Grid de líneas — converge al punto de fuga central
      // Líneas longitudinales (hacia el punto de fuga)
      for (let i = -15; i <= 15; i++) {
        const line = BABYLON.MeshBuilder.CreateBox(`gl${i}`, {
          width: 0.04, height: 0.01, depth: 300,
        }, scene);
        line.position = new BABYLON.Vector3(i * 5, 0.005, 0);
        const lineMat = new BABYLON.StandardMaterial(`glm${i}`, scene);
        // Más brillo en el centro — efecto perspectiva de la película
        const brightness = 1 - Math.abs(i) * 0.06;
      ...
      for (let i = -30; i <= 30; i++) {
        const line = BABYLON.MeshBuilder.CreateBox(`gt${i}`, {
          width: 300, height: 0.01, depth: 0.04,
        }, scene);
        line.position = new BABYLON.Vector3(0, 0.005, i * 5);
        const lineMat = new BABYLON.StandardMaterial(`gtm${i}`, scene);
        lineMat.emissiveColor = new BABYLON.Color3(0, 0.5, 0.6);
        line.material = lineMat;
      }

      // ── TORRES DE TRON CITY — negro con bordes de luz ──────────
      // Basadas en la arquitectura de la película: negras con líneas
      const towerDefs = [
        // [x, z, ancho, alto, colorIdx]
        [-20, -35, 6, 35, 0], [-32, -45, 5, 50, 1],
        [-14, -50, 7, 25, 0], [-42, -38, 4, 45, 0],
        [-25, -58, 5, 38, 1], [-10, -65, 4, 55, 0],
        [ 20, -35, 6, 35, 0], [ 32, -45, 5, 50, 1],
        [ 14, -50, 7, 25, 0], [ 42, -38, 4, 45, 0],
        [ 25, -58, 5, 38, 1], [ 10, -65, 4, 55, 0],
      ];
      ...
      towerDefs.forEach(([x, z, w, h, ci], i) => {
        ...
        // Cuerpo negro
        const body = BABYLON.MeshBuilder.CreateBox(`tb${i}`,
          { width: w, height: h, depth: w }, scene);
        body.position = new BABYLON.Vector3(x, h/2, z);
        body.material = makeBlackMat(`tbm${i}`);

        // Aristas verticales — líneas de circuito
        const corners = [
          [w/2, w/2], [w/2, -w/2], [-w/2, w/2], [-w/2, -w/2]
        ];
        corners.forEach(([cx, cz], ci2) => {
          const edge = BABYLON.MeshBuilder.CreateBox(`te${i}_${ci2}`,
            { width: 0.06, height: h, depth: 0.06 }, scene);
          edge.position = new BABYLON.Vector3(x + cx, h/2, z + cz);
          edge.material = makeMat(`tem${i}_${ci2}`, r, g, b);
        });

        // Corona superior — línea horizontal brillante
        const crown = BABYLON.MeshBuilder.CreateBox(`tc${i}`,
          { width: w + 0.2, height: 0.1, depth: w + 0.2 }, scene);
        crown.position = new BABYLON.Vector3(x, h + 0.05, z);
        crown.material = makeMat(`tcm${i}`, r, g * 1.2, b);

        // Líneas de circuito horizontales en el cuerpo
        [h * 0.25, h * 0.5, h * 0.75].forEach((yOff, li) => {
          const circuit = BABYLON.MeshBuilder.CreateBox(`tcirc${i}_${li}`,
            { width: w + 0.06, height: 0.04, depth: 0.04 }, scene);
          circuit.position = new BABYLON.Vector3(x, yOff, z + w/2 + 0.02);
          circuit.material = makeMat(`tcircm${i}_${li}`, r, g * 0.6, b * 0.6);
        });
      });

      // Torre central al fondo — la más alta, punto de referencia
      const mainTower = BABYLON.MeshBuilder.CreateBox("mainTower",
        { width: 8, height: 120, depth: 8 }, scene);
      mainTower.position = new BABYLON.Vector3(0, 60, -120);
      mainTower.material = makeBlackMat("mainTowerM");

      // Aristas de la torre principal
      [[4,4],[4,-4],[-4,4],[-4,-4]].forEach(([cx,cz], i) => {
        const e = BABYLON.MeshBuilder.CreateBox(`mte${i}`,
          { width: 0.1, height: 120, depth: 0.1 }, scene);
        e.position = new BABYLON.Vector3(cx, 60, -120 + cz);
        e.material = makeMat(`mtem${i}`, 0, 0.97, 1);
      });

      // ── MOTOS TRON LEGACY ──────────────────────────────────────
      // Diseño basado en la película: baja, aerodinámica, con disco
      const motorNodes  = [];
      const motorLights = [];
      ...
      MOTOR_POSITIONS.forEach((pos, i) => {
        ...
        // Cuerpo principal — negro con perfil bajo
        const body = BABYLON.MeshBuilder.CreateBox(`mb${i}`,
          { width: 0.55, height: 0.32, depth: 2.4 }, scene);
        body.parent   = node;
        body.material = makeBlackMat(`mbm${i}`);

        // Franja de identidad lateral — línea de circuito
        const stripe = BABYLON.MeshBuilder.CreateBox(`ms${i}`,
          { width: 0.03, height: 0.03, depth: 2.4 }, scene);
        stripe.parent   = node;
        stripe.position = new BABYLON.Vector3(0.29, 0.1, 0);
        stripe.material = makeMat(`msm${i}`, r, g, b);

        const stripe2 = BABYLON.MeshBuilder.CreateBox(`ms2${i}`,
          { width: 0.03, height: 0.03, depth: 2.4 }, scene);
        stripe2.parent   = node;
        stripe2.position = new BABYLON.Vector3(-0.29, 0.1, 0);
        stripe2.material = makeMat(`ms2m${i}`, r, g, b);

        // Cabina — negro translúcido con borde
        const cabin = BABYLON.MeshBuilder.CreateBox(`mc${i}`,
          { width: 0.38, height: 0.22, depth: 0.75 }, scene);
        cabin.parent   = node;
        ...
        // Ruedas — delgadas y con brillo
        [0.95, -0.95].forEach((zo, wi) => {
          const wheel = BABYLON.MeshBuilder.CreateTorus(`mw${i}_${wi}`,
            { diameter: 0.65, thickness: 0.06, tessellation: 32 }, scene);
          wheel.parent   = node;
        ...
        // Disco de identidad trasero — igual que en la película
        const disc = BABYLON.MeshBuilder.CreateTorus(`md${i}`,
          { diameter: 0.45, thickness: 0.04, tessellation: 32 }, scene);
        disc.parent   = node;
        ...
        // Luz neón desde el suelo — ilumina el grid bajo la moto
        const ml = new BABYLON.PointLight(`ml${i}`,
          new BABYLON.Vector3(pos[0], 0.05, pos[2]), scene);
        ml.intensity = 3;
      ...
      // ── PROGRAMAS (bots) — traje negro con circuitos neón ──────
      // Referencia directa a los personajes de la película
      const botsData   = [...bots].sort((a,b) => b.speed - a.speed);
      const botNodes   = [];
      const botTargets = [];
      const botDoneArr = [];

      const makeTronSuit = (node, r, g, b, BABYLON) => {
        ...
        const torso = BABYLON.MeshBuilder.CreateBox("torso",
          { width: 0.32, height: 0.48, depth: 0.2 }, scene);
        torso.parent = node; torso.position.y = 0.88;
        torso.material = black;

        // Línea de circuito central — pecho
        const chest = BABYLON.MeshBuilder.CreateBox("chest",
          { width: 0.04, height: 0.35, depth: 0.02 }, scene);
        chest.parent = node; chest.position = new BABYLON.Vector3(0, 0.92, 0.11);
        chest.material = neon;

        // Líneas de hombros
        [-0.15, 0.15].forEach((xOff, si) => {
          const shoulder = BABYLON.MeshBuilder.CreateBox(`sh${si}`,
            { width: 0.02, height: 0.02, depth: 0.2 }, scene);
          shoulder.parent = node;
          shoulder.position = new BABYLON.Vector3(xOff, 1.1, 0);
          shoulder.material = neon;
        });

        // Cabeza negra con visor
        const head = BABYLON.MeshBuilder.CreateBox("head",
          { width: 0.22, height: 0.24, depth: 0.22 }, scene);
        head.parent = node; head.position.y = 1.44;
        head.material = black;

        // Visor neón
        const visor = BABYLON.MeshBuilder.CreateBox("visor",
          { width: 0.18, height: 0.06, depth: 0.02 }, scene);
        visor.parent = node;
        ...
        [-0.09, 0.09].forEach((xOff, li) => {
          const leg = BABYLON.MeshBuilder.CreateBox(`leg${li}`,
            { width: 0.13, height: 0.46, depth: 0.14 }, scene);
          leg.parent = node;
          leg.position = new BABYLON.Vector3(xOff, 0.32, 0);
          leg.material = black;

          // Línea de circuito en pierna
          const legLine = BABYLON.MeshBuilder.CreateBox(`ll${li}`,
            { width: 0.02, height: 0.4, depth: 0.02 }, scene);
          legLine.parent = node;
          legLine.position = new BABYLON.Vector3(xOff, 0.32, 0.08);
          legLine.material = neon;
        });

        // Disco de identidad en la espalda — igual que la película
        const disc = BABYLON.MeshBuilder.CreateTorus("disc",
          { diameter: 0.28, thickness: 0.025, tessellation: 32 }, scene);
        disc.parent = node;
      ...
      botsData.forEach((bot, i) => {
        ...
        makeTronSuit(node, r, g, b, BABYLON);

        // Luz de identidad
        const bl = new BABYLON.PointLight(`bl${i}`,
        ...
        botTargets.push(new BABYLON.Vector3(
          MOTOR_POSITIONS[Math.min(i, MOTOR_POSITIONS.length-1)][0], 0,
      ...
      // Loop de bots con animación de carrera
      let runT = 0;
      scene.registerBeforeRender(() => {
          ...
          if (dist < 1.0) {
            ...
            if (!botMotorMap.current[bot.id]) {
              ...
              if (idx >= 9) {
                ...
                setMotorsTaken(prev => { motorsTakenRef.current=prev+1; return prev+1; });
              }
            }
            return;
          }
      ...
      // Colisión jugador con moto
      scene.registerBeforeRender(() => {
        if (playerDoneRef.current || motorsTakenRef.current >= 9) return;
        // Luz del jugador sigue a la cámara
        ...
        MOTOR_POSITIONS.forEach((mp, i) => {
          ...
          const mpos = new BABYLON.Vector3(mp[0], mp[1], mp[2]);
          if (BABYLON.Vector3.Distance(camera.position, mpos) < 1.4) {
            takenSet.current.add(i);
            playerDoneRef.current = true;
            if (motorNodes[i]) motorNodes[i].setEnabled(false);
            if (motorLights[i]) motorLights[i].setEnabled(false);
            playerTakeMotor();
            setPlayerDone(true);
            setMotorsTaken(prev => { motorsTakenRef.current=prev+1; return prev+1; });
            setMessage("¡MOTO TOMADA! ESPERANDO RESULTADO...");
          }
        });
      });

      // ── MANOS DEL JUGADOR — traje Tron con circuito ────────────
      const lHand = BABYLON.MeshBuilder.CreateBox("lhand",
        { width: 0.09, height: 0.055, depth: 0.24 }, scene);
      ...
      // Circuito en cada mano — línea neón cyan
      ["l","r"].forEach((side, si) => {
        const circuit = BABYLON.MeshBuilder.CreateBox(`hc${side}`,
          { width: 0.006, height: 0.006, depth: 0.24 }, scene);
      ...
      // Manos siguen la cámara en primera persona
      let bobT = 0;
      scene.registerBeforeRender(() => {
      ...
      // ── PARTÍCULAS DE DATOS — igual que en la cápsula ──────────
      ...
      ps.minSize    = 0.01; ps.maxSize = 0.045;
      ...
      ps.minEmitPower = 0.1; ps.maxEmitPower = 0.5;
    ...
    <div style={{ width:"100vw", height:"100vh", background:"#000000", position:"relative" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          borderLeft:"2px solid rgba(0,247,255,0.3)",
          paddingLeft:12,
        }}>
        ...
        <div style={{
          ...
          borderRight:"2px solid rgba(0,247,255,0.3)",
          paddingRight:12,
        }}>
      ...
      <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", pointerEvents:"none" }}>
        <div style={{ position:"absolute", top:"50%", left:-10, right:-10, height:"1px", background:"rgba(0,247,255,0.4)", transform:"translateY(-50%)" }}/>
        <div style={{ position:"absolute", left:"50%", top:-10, bottom:-10, width:"1px", background:"rgba(0,247,255,0.4)", transform:"translateX(-50%)" }}/>
        <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", width:4, height:4, borderRadius:"50%", background:"rgba(0,247,255,0.7)" }}/>
      </div>
      ...
      {!message && (
        <div style={{
          ...
          color:"rgba(0,247,255,0.7)", letterSpacing:"0.25em", textAlign:"center",
      ...
      {message && (
        <div style={{
          ...
          textShadow:`0 0 30px ${message.includes("TOMADA") ? "#00f7ff" : "#ff0055"}, 0 0 60px ${message.includes("TOMADA") ? "#00f7ff44" : "#ff005544"}`,
