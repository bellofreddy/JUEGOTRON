...
export default function Phase2({ onComplete }) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // ── LUCES ──────────────────────────────────────────────────
      const roadL = new BABYLON.PointLight(
        "rl",
        new BABYLON.Vector3(0, 0.1, -50),
        scene,
      );
      roadL.intensity = 8;
      roadL.diffuse = new BABYLON.Color3(0, 0.97, 1);
      roadL.range = 130;

      const cliffL = new BABYLON.PointLight(
        "cl",
        new BABYLON.Vector3(0, 3, -200),
        scene,
      );
      cliffL.intensity = 20;
      cliffL.diffuse = new BABYLON.Color3(0, 0.97, 1);
      cliffL.range = 90;
      ...
      [-9, 9].forEach((x, i) => {
        const e = BABYLON.MeshBuilder.CreateBox(
          `re${i}`,
          { width: 0.07, height: 0.07, depth: 230 },
          scene,
        );
      ...
      for (let i = 0; i < 42; i++) {
        const d = BABYLON.MeshBuilder.CreateBox(
          `d${i}`,
          { width: 0.04, height: 0.02, depth: 2.8 },
          scene,
        );
      ...
      for (let i = 0; i < 46; i++) {
        const c = BABYLON.MeshBuilder.CreateBox(
          `c${i}`,
          { width: 18, height: 0.01, depth: 0.04 },
          scene,
        );
      ...
      [-10, 10].forEach((x, si) => {
        for (let j = 0; j < 22; j++) {
          const v = BABYLON.MeshBuilder.CreateBox(
            `vl${si}_${j}`,
            { width: 0.035, height: 55, depth: 0.035 },
            scene,
          );
      ...
      // Fondo del abismo
      const abyss = BABYLON.MeshBuilder.CreateBox(
        "ab",
        { width: 500, height: 0.1, depth: 500 },
        scene,
      );
      abyss.position.y = -58;
      ...
      const cliffEdge = BABYLON.MeshBuilder.CreateBox(
        "ce",
        { width: 18, height: 0.4, depth: 0.25 },
        scene,
      );
      ...
      TUBE_X.forEach((x, i) => {
        ...
        // Cable
        const cable = BABYLON.MeshBuilder.CreateBox(
          `cab${i}`,
          { width: 0.025, height: 7, depth: 0.025 },
          scene,
        );
        cable.parent = node;
        ...
        const body = BABYLON.MeshBuilder.CreateCylinder(
          `tb${i}`,
          { height: 3.2, diameter: 0.32, tessellation: 16 },
          scene,
        );
        body.parent = node;
        body.material = black(`tbM${i}`);

        // Anillos de circuito
        [1.1, 0.2, -0.7, -1.5].forEach((y, ri) => {
          const ring = BABYLON.MeshBuilder.CreateTorus(
            `tr${i}_${ri}`,
            { diameter: 0.42, thickness: 0.038, tessellation: 24 },
            scene,
          );
          ring.parent = node;
          ring.position.y = y;
        ...
        const grip = BABYLON.MeshBuilder.CreateBox(
          `tg${i}`,
          { width: 0.48, height: 0.1, depth: 0.48 },
          scene,
        );
        grip.parent = node;
        grip.position.y = -1.7;
        ...
        // Luz
        const tl = new BABYLON.PointLight(
          `tl${i}`,
          new BABYLON.Vector3(x, 3, -220),
          scene,
        );
        tl.intensity = 5;
        tl.diffuse = new BABYLON.Color3(r, g, b);
        tl.range = 7;
      ...
      const pmB = BABYLON.MeshBuilder.CreateBox(
        "pmB",
        { width: 0.5, height: 0.28, depth: 2.2 },
        scene,
      );
      pmB.parent = pMoto;
      pmB.material = black("pmBM");

      const pmS = BABYLON.MeshBuilder.CreateBox(
        "pmS",
        { width: 0.025, height: 0.025, depth: 2.2 },
        scene,
      );
      pmS.parent = pMoto;
      pmS.position.x = 0.27;
      ...
      [0.85, -0.85].forEach((zo, wi) => {
        const w = BABYLON.MeshBuilder.CreateTorus(
          `pmW${wi}`,
          { diameter: 0.55, thickness: 0.055, tessellation: 20 },
          scene,
        );
        w.parent = pMoto;
        w.position = new BABYLON.Vector3(0, -0.12, zo);
        w.rotation.z = Math.PI / 2;
        w.material = neon(`pmWM${wi}`, 0, 0.97, 1);
      });

      // ── MANOS ─────────────────────────────────────────────────
      const lH = BABYLON.MeshBuilder.CreateBox(
        "lH",
        { width: 0.08, height: 0.05, depth: 0.22 },
        scene,
      );
      lH.material = black("lHM");
      const lC = BABYLON.MeshBuilder.CreateBox(
        "lC",
        { width: 0.005, height: 0.005, depth: 0.22 },
        scene,
      );
      lC.material = neon("lCM", 0, 0.97, 1);

      const rH = BABYLON.MeshBuilder.CreateBox(
        "rH",
        { width: 0.08, height: 0.05, depth: 0.22 },
        scene,
      );
      rH.material = black("rHM");
      const rC = BABYLON.MeshBuilder.CreateBox(
        "rC",
        { width: 0.005, height: 0.005, depth: 0.22 },
        scene,
      );
      ...
      botsAlive.forEach((bot, i) => {
        ...
        const mb = BABYLON.MeshBuilder.CreateBox(
          `bmb${i}`,
          { width: 0.45, height: 0.25, depth: 2.0 },
          scene,
        );
        mb.parent = node;
        mb.material = black(`bmbM${i}`);

        const ms = BABYLON.MeshBuilder.CreateBox(
          `bms${i}`,
          { width: 0.022, height: 0.022, depth: 2.0 },
          scene,
        );
        ms.parent = node;
        ms.position.x = 0.24;
        ...
        [0.8, -0.8].forEach((zo, wi) => {
          const w = BABYLON.MeshBuilder.CreateTorus(
            `bmw${i}_${wi}`,
            { diameter: 0.5, thickness: 0.05, tessellation: 16 },
            scene,
          );
          w.parent = node;
          w.position = new BABYLON.Vector3(0, -0.11, zo);
          w.rotation.z = Math.PI / 2;
      ...
      let startJumpPos = null; // <--- ESTA ES LA QUE FALTA
      let closestIdx = -1; // <--- ESTA TAMBIÉN SE NECESITA
      let jumpProgress = 0; // <--- PARA EL TIEMPO DEL SALTO
      ...
      scene.registerBeforeRender(() => {
        ...
        // ── RIDING — CONTROL MANUAL + BORDES PRECISOS ──────────────
        if (gPhase === "RIDING") {
          // 1. Velocidad Manual
          let targetSpeed = 22;
          if (Input.isDown("KeyW")) targetSpeed = 46;
          if (Input.isDown("KeyS")) targetSpeed = 10;

          rideSpd = BABYLON.Scalar.Lerp(rideSpd, targetSpeed, dt * 2.5);
          rideZ -= rideSpd * dt;

          // 2. Movimiento Lateral
          const lateralSpeed = 11;
          if (Input.isDown("KeyA")) camera.position.x += lateralSpeed * dt;
          if (Input.isDown("KeyD")) camera.position.x -= lateralSpeed * dt;

          // 3. VALIDACIÓN DE IMPACTO (LOS LÍMITES REALES)
          // El neón está en 9 y -9. 8.9 asegura que mueras justo al tocar la luz.
          const LIMIT = 8.9;
          if (Math.abs(camera.position.x) > LIMIT) {
            gPhase = "DONE";
            setPlayerDone(true);
            setMessage("¡DESREZZADO: IMPACTO CRÍTICO!");

            // ── EFECTO DE EXPLOSIÓN / GLITCH ──
          ...
          const targetTilt =
            (Input.isDown("KeyD") ? -0.15 : 0) +
            (Input.isDown("KeyA") ? 0.15 : 0);
        ...
        // ── JUMPING ───────────────────────────────────────────────
        if (gPhase === "JUMPING") {
          // 1. Capturar la posición inicial SOLO el primer frame del salto
          if (!startJumpPos) {
            // Ahora sí existirá porque la declaramos arriba
            startJumpPos = camera.position.clone();

            let minD = 999;
            TUBE_X.forEach((tx, i) => {
              let d = Math.abs(camera.position.x - tx);
              if (d < minD) {
                minD = d;
                closestIdx = i; // Guardamos el índice del tubo más cercano
              }
            });
          }
          ...
          // 2. Interpolación de posición (Parábola)
          const targetX = TUBE_X[closestIdx];
          ...
          const parabola = 1 - Math.pow(2 * p - 1, 2);
          camera.position.y =
            startJumpPos.y + parabola * jumpHeight + p * (5.0 - startJumpPos.y);
      ...
      >
        ...
        >
          ...
          <span style={{ color: "#ff6600" }}>
            TUBOS: {Math.max(0, 8 - tubesTaken)}/8
          </span>
        </div>
        <div
          style={{
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              textShadow: "0 0 20px #00f7ff",
              boxShadow:
                "0 0 30px rgba(0,247,255,0.2), inset 0 0 20px rgba(0,247,255,0.05)",
            }}
          >
            ▶ MONTAR
          </button>
...
function _updateHands(camera, lH, lC, rH, rC, BABYLON, running) {
  ...
  lH.position = camera.position
    .clone()
    .subtract(rt.scale(0.22))
    .subtract(up.scale(0.19))
...
function _updateBots(
  ...
  tubeLights,
  botTubes,
  tubeCount, // Este ahora lo usaremos como referencia de índice
  setCount,
  eliminate,
  setMsg,
  setSurv,
  ...
  bots.forEach((bot, i) => {
    ...
    if (states[i] === "JUMPING") {
      ...
      if (Math.abs(node.position.z - tubesZ) < 1.5) {
        ...
        // ... dentro de _updateBots ...
        if (!botTubes[bot.id]) {
          const currentGlobalTubes = useBattleStore.getState().tubesTaken;
          botTubes[bot.id] = true;

          // Solo permitimos que los bots tomen tubos si hay 7 o menos ocupados
          if (currentGlobalTubes < 7) {
            if (tubeNodes[currentGlobalTubes])
              tubeNodes[currentGlobalTubes].setEnabled(false);
            if (tubeLights[currentGlobalTubes])
              tubeLights[currentGlobalTubes].setEnabled(false);

            useBattleStore.getState().botTakeTube(bot.id);
            setCount(currentGlobalTubes + 1);
          } else {
            // El 8vo bot (y los siguientes) ya no encuentran tubo y se desrezzan
            eliminate(bot.id);
